    ...
    try:
        ...
        for line in lines:
            ...
            # 2. Cerca canale - pattern CORRETTO!
                ...
                # Deve essere PRECISO!
                src_match = re.search(
                    r'>([0-9]:[0-9]:[0-9A-F]+:[0-9A-F]+:[0-9A-F]+:[0-9A-F]+:[0-9A-F]+:[0-9]:[0-9]:[0-9]:)<',
                    line)
                ...
                if src_match:
                    ...
                    if name_match:
                        ...
                        channel_name = id_match.group(
                            1) if id_match else "Unknown"

                    # Salva
                    xml_mappings[src_png]['name'] = channel_name
                    if current_satellite:
                        xml_mappings[src_png]['satellites'].add(
                            current_satellite)
        ...
        for src, info in xml_mappings.items():
            if 'EEEE0000' in src and count < 3:
                print(
                    f"  {src} -> '{info['name']}' - {info['satellites_str']}")
                count += 1
...
def generate_mapping():
    ...
    print("RESULTS:")
    print(
        f"Found: {found_count}/{len(png_srcs)} ({found_count / len(png_srcs) * 100:.1f}%)")
    print(
        f"Not found: {
            len(not_found_list)}/{
            len(png_srcs)} ({
                len(not_found_list) / len(png_srcs) * 100:.1f}%)")
