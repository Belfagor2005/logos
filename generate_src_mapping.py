...
def parse_lamedb(content):
    ...
    while i < len(lines):
        ...
        if re.match(
            r'^[0-9a-fA-F]+:[0-9a-fA-F]+:[0-9a-fA-F]+:[0-9a-fA-F]+:[0-9]:[0]$',
                line):
            src_enigma = line
            ...
            if match:
                ...
                while j < len(lines) and j < i + 5:  # Check next 5 lines
                    ...
                    if next_line.startswith('#DESCRIPTION'):
                        channel_name = next_line.replace(
                            '#DESCRIPTION', '').strip()
...
def convert_enigma_to_png(enigma_src):
    ...
    try:
        ...
        src_png = "1_0_" + service_id + "_" + transport_stream_id + \
            "_" + original_network_id + "_" + namespace + "_0_0_0"
...
def generate_complete_mapping():
    ...
    with open('src_mapping.txt', 'w', encoding='utf-8') as f:
        ...
        f.write(
            "# Generated from: rytec.channels.xml + Vhannibal Motor + PNG directories\n")
    ...
    with open('picons_missing.txt', 'w', encoding='utf-8') as f:
        ...
        f.write(
            "# These channels exist in XML/Vhannibal but don't have PNG files in repository\n")
        ...
        for item in missing_picons_sorted:
            ...
            f.write(
                "Suggested path: logos/E2LIST/Unknown/" +
                item['src'] +
                ".png\n")
            f.write("-" * 50 + "\n\n")
    ...
    with open('summary.txt', 'w', encoding='utf-8') as f:
        ...
        f.write("SRCs from XML: " +
                str(len([v for v in all_channel_data.values() if v['source'] == 'xml'])) +
                "\n")
        f.write("SRCs from Vhannibal: " +
                str(len([v for v in all_channel_data.values() if v['source'] == 'vhannibal'])) +
                "\n")
        f.write("Total unique SRCs from all sources: " +
                str(len(all_channel_data)) + "\n\n")

        f.write("XML matches: " + str(xml_matches) + " (" +
                str(round(xml_matches / len(png_srcs) * 100, 1)) + "%)\n")
        f.write("Vhannibal matches: " + str(vhannibal_matches) + " (" +
                str(round(vhannibal_matches / len(png_srcs) * 100, 1)) + "%)\n")
        f.write("Unknown SRCs: " + str(unknown_count) + " (" +
                str(round(unknown_count / len(png_srcs) * 100, 1)) + "%)\n")
    ...
    print("Vhannibal matches: " + str(vhannibal_matches) +
          " (" + str(round(vhannibal_matches / len(png_srcs) * 100, 1)) + "%)")
    print("Unknown SRCs: " + str(unknown_count) +
          " (" + str(round(unknown_count / len(png_srcs) * 100, 1)) + "%)")
    print("TOTAL COVERAGE: " + str(round(coverage, 1)) + "%")
    print("PICONS MISSING: " +
          str(len(missing_picons)) +
          " (check picons_missing.txt)")
