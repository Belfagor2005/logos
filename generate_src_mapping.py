...
def generate_final_mapping():
    ...
    with open('src_mapping.txt', 'w', encoding='utf-8') as f:
        ...
        f.write(
            f"# Direct matches in XML: {len([l for l in mapping_lines if '(similar)' not in l and 'UNKN' not in l])}\n")
        f.write(f"# Similarity matches: {matched_by_similarity}\n")
        f.write(
            f"# Unknown: {len([l for l in mapping_lines if 'UNKN' in l]) - matched_by_similarity}\n")
    ...
    unknown_count = len(
        [l for l in mapping_lines if 'UNKN' in l]) - similarity_matches
    ...
    print(
        f"📈 Exact matches in XML: {
            exact_matches:,} ({
            exact_matches / len(png_srcs) * 100:.1f}%)")
    print(
        f"📉 Similarity matches: {
            similarity_matches:,} ({
            similarity_matches / len(png_srcs) * 100:.1f}%)")
    print(
        f"❓ Unknown (need research): {
            unknown_count:,} ({
            unknown_count / len(png_srcs) * 100:.1f}%)")
    ...
    print(
        f"\n🔍 For {
            unknown_count:,} unknown channels, check missing_pngs_detailed.txt")
