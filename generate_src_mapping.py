...
def analyze_png_and_xml():
    ...
    for i, src in enumerate(png_srcs, 1):
        ...
        for j, part in enumerate(parts, 1):
            print(f"   [{j}] {part} (hex: {part if all(c in '0123456789ABCDEF' for c in part.upper()) else 'no'})")
        ...
        if xml_format in xml_data:
            ...
            if len(parts) >= 7:
                ...
                try:
                    ...
                    variant1 = f"{parts[0]}:{parts[1]}:{parts[2]}:{sid_hex}:{tsid_hex}:{onid_hex}:{parts[6]}:{parts[7]}:{parts[8]}:{parts[9]}:"
                    variants.append(variant1)
                    
                    variant2 = f"{parts[0]}:{parts[1]}:{parts[2]}:{sid_dec}:{tsid_dec}:{onid_dec}:{parts[6]}:{parts[7]}:{parts[8]}:{parts[9]}:"
        ...
        if found_matches:
            ...
            for xml_src, line in found_matches[:2]:  # Mostra max 2
                ...
                if len(xml_parts) >= 7 and len(parts_png) >= 7:
                    print(f"    Compare:")
                    print(f"      PNG[{3}]: {parts_png[2]} vs XML[{3}]: {xml_parts[2]}")  # SID
                    print(f"      PNG[{6}]: {parts_png[5]} vs XML[{6}]: {xml_parts[5]}")  # ONID
                    print(f"      PNG[{7}]: {parts_png[6]} vs XML[{7}]: {xml_parts[6]}")  # Provider
        else:
