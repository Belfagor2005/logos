...
def generate_final_mapping():
    ...
    multi_sat_png_files = sum(1 for sats in png_file_to_satellites.values() if len(sats) > 1)
    ...
    for src_png in sorted(png_srcs.keys()):
        ...
        if match_type == "similar":
            mapping_lines.append(f"{src_png} - {snp_code} - {satellites_str} # SIMILAR")
        else:
    ...
    with open('src_mapping.txt', 'w', encoding='utf-8') as f:
        ...
        f.write(f"# PNG files on multiple satellites: {multi_sat_png_files:,}\n")
        ...
        f.write("# Lines ending with # SIMILAR are educated guesses based on similar SRC patterns\n\n")
    ...
    with open('missing_pngs_analysis.txt', 'w', encoding='utf-8') as f:
        ...
        f.write(f"# With similarity suggestions: {len([m for m in missing_pngs if m['match_type'] == 'similar']):,}\n")
        f.write(f"# Completely unknown: {len([m for m in missing_pngs if m['match_type'] == 'unknown']):,}\n")
        f.write("# Generated: " + time.strftime('%Y-%m-%d %H:%M:%S') + "\n\n")
        
        # Separa per tipo
        similar_matches = [m for m in missing_pngs if m['match_type'] == 'similar']
        unknown_matches = [m for m in missing_pngs if m['match_type'] == 'unknown']
        
        if similar_matches:
            f.write(f"## SIMILARITY MATCHES (likely correct): {len(similar_matches):,}\n")
            f.write("# These SRCs match similar patterns in XML and are probably correct\n")
            ...
            for item in similar_matches[:50]:  # Limita a 50 esempi
                ...
                f.write(f"SUGGESTED: {item['suggested_name']} (similarity: {item['similarity']}/6)\n")
                f.write(f"Based on XML SRC: {item['similar_src']}\n")
                f.write(f"Action: Add to XML as: <!-- sat --><channel id=\"ID\">SRC</channel><!-- {item['suggested_name']} -->\n")
                f.write("-" * 50 + "\n\n")
            
            if len(similar_matches) > 50:
                f.write(f"... and {len(similar_matches) - 50} more similarity matches\n\n")
        ...
        if unknown_matches:
            ...
            for item in unknown_matches[:30]:  # Limita a 30 esempi
                ...
                if item.get('suggestions'):
                    ...
                    for sug in item['suggestions'][:2]:
                        ...
                        f.write(f"  • {sug['name']} (similarity: {sug['similarity']}/6, SRC: {sug['src']})\n")
            ...
            if len(unknown_matches) > 30:
                f.write(f"... and {len(unknown_matches) - 30} more unknown SRCs\n\n")
    ...
    coverage_percentage = (exact_matches + similarity_matches) / total_png_srcs * 100
    
    print(f"✅ Processing completed in {time.time() - start_time:.1f} seconds")
    print(f"📊 Total PNG SRCs processed: {total_png_srcs:,}")
    print(f"📈 Exact matches in XML: {exact_matches:,} ({exact_matches/total_png_srcs*100:.1f}%)")
    print(f"📉 Similarity matches: {similarity_matches:,} ({similarity_matches/total_png_srcs*100:.1f}%)")
    print(f"❓ Unknown (need research): {unknown_count:,} ({unknown_count/total_png_srcs*100:.1f}%)")
    print(f"✅ Total coverage: {coverage_percentage:.1f}%")
    print(f"🌍 Multi-satellite channels found: {len([l for l in mapping_lines if '|' in l]):,}")
