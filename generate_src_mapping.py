...
def generate_mapping():
    ...
    for src_png, info in xml_data.items():
        if src_png not in png_srcs:
            missing_picons.append({'src': src_png, 'name': info['name'], 'satellites': list(
                info['satellites'])[0] if info['satellites'] else 'Unknown'})
    ...
    try:
        with open('src_mapping.txt', 'w', encoding='utf-8') as f:
            f.write(
                "# SRC (codice) - SNP (codice abbreviato del nome del canale) - posizioni satellitari disponibili\n")
            ...
            f.write(
                "# Format example: 1_0_16_105_F01_20CB_EEEE0000_0_0_0 - filmbox premium - 23.5E|16.0E|13.0E|0.8W\n\n")
    ...
    try:
        with open('picons_missing.txt', 'w', encoding='utf-8') as f:
            ...
            f.write("# =============================\n")
            f.write("# Total missing picons: " +
                    str(len(missing_picons)) + "\n")
            f.write(
                "# These channels exist in XML but don't have PNG files in repository\n")
            ...
            for letter in sorted(missing_by_letter.keys()):
                f.write("\n## LETTER " + letter + " (" +
                        str(len(missing_by_letter[letter])) + " channels)\n\n")

                for item in sorted(
                        missing_by_letter[letter],
                        key=lambda x: x['name']):
