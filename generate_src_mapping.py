...
def generate_mapping():
    ...
    with open('src_mapping.txt', 'w', encoding='utf-8') as f:
        ...
        f.write(f"# Unique PNG SRCs: {len(png_srcs)}, Found in XML: {found_count}, Not found: {len(not_found_list)}\n")
        f.write(f"# Note: Same SRC may appear multiple times with different satellite positions\n\n")
    ...
    with open('not_found_src.txt', 'w', encoding='utf-8') as f:
        ...
        f.write(f"# Total PNG files not found (with duplicates): {sum(png_srcs[src] for src in not_found_list)}\n")
    ...
    print(f"Found in XML: {found_count} ({found_count/len(png_srcs)*100:.1f}% of unique SRCs)")
    print(f"Not found in XML: {len(not_found_list)} ({len(not_found_list)/len(png_srcs)*100:.1f}% of unique SRCs)")
