...
def download_vhannibal():
    ...
    try:
            ...
            with zipfile.ZipFile(io.BytesIO(response.content)) as zip_file:
                        ...
                        if ':' in line and len(line.split(':')) == 6:
                            ...
                            if len(parts[0]) == 4 and len(parts[1]) == 8:
                                ...
                                if i + 1 < len(lines):
                                    ...
                                    if channel_name and not channel_name.startswith('p:'):
                                        ...
                                        try:
                                            ...
                                            src_png = "1_0_" + service_id + "_" + transport_id + "_" + network_id + "_" + namespace + "_0_0_0"
                ...
                for filename in zip_file.namelist():
                    if filename.endswith('.tv'):
                        ...
                        # Parse #SERVICE lines
                        service_lines = re.findall(r'#SERVICE\s+([0-9a-fA-F:]+)', tv_content)
                        for service in service_lines:
                            ...
                            if service.count(':') == 5:
                                ...
                                try:
                                    ...
                                    name_match = re.search(r'#DESCRIPTION\s+(.+)', tv_content)
                                    if name_match:
        ...
        print("Found " + str(len(vhannibal_data)) + " unique channels in Vhannibal")
...
def generate_complete_mapping():
        ...
        if missing_picons:
            with open('missing_picons.txt', 'w', encoding='utf-8') as f:
                ...
                for source in sorted(missing_by_source.keys()):
                    ...
                    f.write("\n## " + source.upper() + " (" + str(len(items)) + " channels)\n\n")
            ...
            print("✓ Created: missing_picons.txt (" + str(len(missing_picons)) + " entries)")
        else:
            with open('missing_picons.txt', 'w', encoding='utf-8') as f:
    ...
    try:
        with open('summary.txt', 'w', encoding='utf-8') as f:
            ...
            f.write("# =======\n")
            f.write("Generated: " + time.strftime('%Y-%m-%d %H:%M:%S') + "\n")
            f.write("Time: " + str(round(time.time() - start_time, 1)) + " seconds\n\n")
            
            f.write("PNG SRCs in repository: " + str(len(png_dict)) + "\n")
            f.write("Channels in XML: " + str(len([v for v in all_channels.values() if v['source'] == 'xml'])) + "\n")
            f.write("Channels in Vhannibal: " + str(len([v for v in all_channels.values() if v['source'] in ['vhannibal', 'vhannibal_tv']])) + "\n")
            f.write("Total unique channels: " + str(len(all_channels)) + "\n\n")
    ...
    if missing_picons:
        print("\nNEXT: Create " + str(len(missing_picons)) + " missing PNG files")
