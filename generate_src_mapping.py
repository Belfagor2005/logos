...
def create_snp_code(channel_name):
    ...
    clean = re.sub(r'\s*(?:HD|FHD|UHD|4K|SD|\.\w{2,4})$', '', channel_name, flags=re.IGNORECASE)
...
def generate_final_mapping():
    ...
    for src in sorted(srcs_only_in_png):
        ...
        if src_analysis:
            line += f"Analysis: {src_analysis['service_type']}, Service ID: {src_analysis['service_id']}, "
            line += f"TS ID: {src_analysis['ts_id']}, ONID: {src_analysis['onid']}\n"
    ...
    with open('missing_pngs_detailed.txt', 'w', encoding='utf-8') as f:
        ...
        f.write("# These Service References have PNG files but NO entry in rytec.channels.xml\n")
        f.write("#\n")
        f.write("# HOW TO FIX:\n")
    ...
    print(f"\nCoverage: {len(srcs_in_both)/len(all_srcs)*100:.1f}% complete")
    
    # Esempi
    if mapping_lines:
        print(f"\nSample from src_mapping.txt (first 3 with channel names):")
        named_samples = [line for line in mapping_lines if ' - UNKN - ' not in line]
    ...
    if srcs_only_in_png:
        ...
        if sample_missing in png_srcs:
            print(f"  Satellites: {', '.join(sorted(png_srcs[sample_missing]))}")
    
    print(f"\nFiles created:")
    print(f"  1. src_mapping.txt - Main mapping ({(len(mapping_lines)/1024):.1f}KB)")
