...
def parse_xml_for_channels():
    ...
    try:
        ...
        # Cattura: <!-- satellite --><channel id="id">src</channel><!-- nome canale -->
        ...
        for match in matches:
            ...
            if after_comment:
                ...
                # Se il commento prima non contiene -->, potrebbe essere il nome
                channel_name = before_comment.strip()
            
            # Pulisci nome canale
            channel_name = re.sub(r'^\d+\.\d+[EW]\s*[-–>]*\s*', '', channel_name)
        ...
        for src, data in samples:
            sats = ', '.join(data['satellites']) if data['satellites'] else 'none'
...
def generate_final_mapping():
    ...
    for src in sorted(srcs_only_in_png):
        ...
        if len(parts) >= 7:
            ...
            entry += f"   Service Type: {parts[0]} ({'TV' if parts[0] == '1' else 'Radio' if parts[0] == '2' else 'Other'})\n"
        ...
        if suggestions:
            ...
            for sug in suggestions:
                entry += f"   • {sug['name']} (similarity: {sug['similarity']:.0%})\n"
                entry += f"     SRC: {sug['src']}\n"
    ...
    with open('src_mapping.txt', 'w', encoding='utf-8') as f:
        ...
        f.write(f"# Known channels: {known_count:,} ({known_count/len(mapping_lines)*100:.1f}%)\n")
        f.write(f"# Unknown channels: {unknown_count:,} ({unknown_count/len(mapping_lines)*100:.1f}%)\n")
    ...
    with open('missing_pngs_detailed.txt', 'w', encoding='utf-8') as f:
        ...
        f.write(f"• Percentage of all SRCs: {len(srcs_only_in_png)/len(all_srcs)*100:.1f}%\n")
    ...
    print(f"📈 Known channels: {known_count:,} ({known_count/len(mapping_lines)*100:.1f}%)")
    print(f"❓ Unknown channels: {unknown_count:,} ({unknown_count/len(mapping_lines)*100:.1f}%)")
    
    # Esempi
    print(f"\n📄 Sample known channels (first 5):")
    known_samples = [line for line in mapping_lines if ' - UNKN - ' not in line]
    ...
    print(f"\n📁 Files created:")
    print(f"  1. 📋 src_mapping.txt - Main mapping file ({len(mapping_lines):,} lines)")
