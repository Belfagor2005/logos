...
def parse_xml_for_channels():
    ...
    try:
        ...
        for match in matches:
            ...
            if len(xml_dict) <= 5:
                print(
                    f"  Parsed: {src_png} -> '{channel_name}' (sat: {satellite})")
...
def create_snp_code(channel_name):
    ...
    clean_name = re.sub(
        r'\s*(?:HD|FHD|UHD|4K|SD|HEVC|H265|H264|\.\w{2,4})$',
        '',
        channel_name,
        flags=re.IGNORECASE)
...
def extract_real_channel_name(comment):
    ...
    name = re.sub(r'\s*(?:HD|FHD|UHD|4K|SD|HEVC)\b',
...
def generate_final_mapping():
    ...
    for src in sorted(xml_dict.keys()):
        ...
        satellites_str = '|'.join(
            sorted_satellites) if sorted_satellites else 'Unknown'
    ...
    with open('missing_pngs.txt', 'w', encoding='utf-8') as f:
        ...
        f.write(
            "# These SRC codes exist in PNG files but not in rytec.channels.xml\n\n")
